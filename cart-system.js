window.addEventListener('load', () => {
    ...
    // 2. Navbar Cart aur My Orders Link
    const navList = document.querySelector('ul'); 
    if (navList) {
        // Cart Link
        if (!document.getElementById('cart-link-nav')) {
            const cartLi = document.createElement('li');
            cartLi.id = "cart-link-nav";
        ...
        // My Orders Link
        if (!document.getElementById('orders-link-nav')) {
            const ordersLi = document.createElement('li');
            ordersLi.id = "orders-link-nav";
    ...
    // 3. Cart Logic (Firebase Auth Check)
